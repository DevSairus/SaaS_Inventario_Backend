  ...
  try {
    ...
    const quantityQuery = `
      ...
      WHERE tenant_id = :tenantId
        AND movement_date >= NOW() - INTERVAL '${parseInt(months)} months'
    ...
    const valueQuery = `
      ...
      WHERE im.tenant_id = :tenantId
        AND im.movement_date >= NOW() - INTERVAL '${parseInt(months)} months'
  ...
  try {
    ...
    const query = `
      SELECT 
        ...
        COALESCE(SUM(si.quantity * p.average_cost), 0)::numeric as total_cost,
        COALESCE(SUM(si.quantity * (si.unit_price - p.average_cost)), 0)::numeric as profit,
        ROUND(
          CASE 
            WHEN SUM(si.quantity * p.average_cost) > 0 THEN
              (SUM(si.quantity * (si.unit_price - p.average_cost)) / 
               SUM(si.quantity * p.average_cost)) * 100
            ELSE 0
          END,
          2
      ...
      WHERE p.tenant_id = :tenantId
        ...
        AND s.sale_date >= NOW() - INTERVAL '${parseInt(months)} months'
...
exports.getRotationReport = async (req, res) => {
  try {
    ...
    const high_rotation = allProducts
      .filter(p => p.rotation_status === 'Alta rotación')
      .slice(0, 10); // Top 10

    const low_rotation = allProducts
      .filter(p => p.rotation_status === 'Sin movimiento' || p.rotation_status === 'Baja rotación')
      .slice(0, 10); // Top 10 sin movimiento
