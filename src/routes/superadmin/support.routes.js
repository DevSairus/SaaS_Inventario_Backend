...
//
// Desde la migración a schema-per-tenant, borrar un tenant ya no es solo
// borrar unas filas en `public` -- la mayoría de sus datos viven en su
// schema dedicado (tenant_<slug>). Ver src/services/tenantPurgeService.js.
//
// Exige confirmación explícita: el body debe traer `confirm` con el nombre
// exacto de la empresa (mismo patrón que GitHub/Vercel para borrados
// irreversibles). Es a propósito más estricto que un simple "¿estás
// seguro?" porque esta acción ya no se puede revertir.
router.delete(
  '/tenants/:id',
  authMiddleware,
  checkPermission('superadmin.manage_all'),
  async (req, res) => {
    try {
      ...
      const expected = tenant.company_name.trim().toLowerCase();
      if (!confirm || confirm.trim().toLowerCase() !== expected) {
        return res.status(400).json({
          error: 'Confirmación inválida. Escribe el nombre exacto de la empresa para confirmar la eliminación permanente.',
        });
      }
