...
async function createSupportDocumentAdjustment(tenant, {
  ...
  let unsignedXml = buildCreditNoteXml(doc, uuid, softwareSecurityCode)
    ...
    .replace(
      /<cbc:ProfileID>[^<]*<\/cbc:ProfileID>/,
      // Ni el espacio final (ejemplo oficial "NotaDeAjuste.xml") ni el punto
      // final (intento anterior, por analogía con el Documento Soporte) eran
      // correctos -- confirmado por un caso real ajeno con el mismo rechazo
      // NSAD03 (foro facturasyrespuestas.com): el literal exacto NO lleva
      // ningún carácter después de "equivalente".
