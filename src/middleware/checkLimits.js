    ...
    try {
      ...
      if (tenant.subscriptionPlan) {
        plan = {
          ...
          max_branches: tenant.subscriptionPlan.max_branches,
          allow_extra_branches: tenant.subscriptionPlan.allow_extra_branches,
      ...
      switch (resourceType) {
        ...
        case 'branches': {
          ...
          currentCount = await Branch.count({
      ...
      // Sedes: si el plan permite sobrecargo, dejar pasar aunque se exceda el
      // límite — la sede extra queda facturable manualmente (ver
      // superadmin.routes.js#GET /tenants/:id), no bloqueada.
      const allowsOverage = resourceType === 'branches' && plan.allow_extra_branches;

      // Verificar si se ha alcanzado el límite
      if (!allowsOverage && currentCount >= maxLimit) {
